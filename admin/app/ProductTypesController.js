...
app.controller('ProductTypesController', ['$scope', '$timeout', 'productTypes', 'notifier', function ($scope, $timeout, productTypes, notifier) {
    ...
    $scope.delete = function (id) {
        noty({
            text: 'Are you sure you want to delete this type?\nAll products for this type will also be deleted!',
            type: 'error',
            dismissQueue: true,
            layout: 'center',
            buttons: [
                {
                    ...
                    $noty.close();
                    $('#container').find('.type-item[id=' + id + ']').attr('src', 'assets/images/loading.gif');
                    productTypes.delete(id, function (success, message) {
                        if (success) {
                            notifier.success("Successfully deleted");
                            $('#container').isotope('remove', $('#container').find('.type-item[id=' + id + ']')).isotope();
                        } else {
                            notifier.alert(message);
                        }
                        $('#container').find('.type-item[id=' + id + ']').attr('src', 'assets/images/clear.png');
                    });
                }
                },
                {
